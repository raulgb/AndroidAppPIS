...
import static edu.ub.pis2016.pis16.strikecom.gameplay.config.GameConfig.GROUP_RAIDERS;
import static edu.ub.pis2016.pis16.strikecom.gameplay.config.GameConfig.TILE_SIZE;

/**
 * Created by Arbitro on 17/05/2016.
 */
	...
	public static GameObject createEnemyTank(){
		...
		final ThreadVehicle enemyTank = new ThreadVehicle() {
			...
			public void destroy() {
				...
				this.getScreen().addGameObject(explosion);

				// Add Scrap to Player
				FragmentedGameActivity gameActivity = (FragmentedGameActivity) ((GLGameFragment) getScreen().getGame()).getActivity();

				int scrap = (int) gameActivity.playerState.get("SCRAP");
				gameActivity.playerState.put("SCRAP", scrap + this.maxHitpoints);

				// TODO Crete destroy listener
